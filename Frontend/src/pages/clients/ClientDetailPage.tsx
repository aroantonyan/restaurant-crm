  ...
  if (error || !client) {
    return (
      <main className="page-enter px-5 pt-4 pb-10 max-w-md mx-auto w-full text-center">
        <p className="text-tg-destructive text-sm">{error ?? t('clients.errors.loadFailed')}</p>
  ...
  return (
    <main className="page-enter flex flex-col px-5 pt-4 pb-10 max-w-md mx-auto w-full min-h-full">
      ...
      <div className="rounded-2xl bg-tg-secondary-bg px-5 py-4 mb-3">
        <p className="text-[11px] text-tg-hint uppercase tracking-wide font-medium">{t('clients.balance')}</p>
        <p className={`text-3xl font-bold mt-1 tabular-nums ${negative ? 'text-tg-destructive' : 'text-tg-text'}`}>
        ...
        {client.loyaltyType === 'Cashback' && client.loyaltyRate > 0 && (
          <p className="text-xs text-tg-text mt-2">
      ...
      {canManage && (
        <div className="grid grid-cols-2 gap-2 mb-5">
          <button type="button" onClick={() => setDialog('deposit')} className="py-3 rounded-xl bg-tg-button text-tg-button-text font-semibold active:scale-[0.98] transition">
            + {t('clients.deposit')}
          </button>
          <button type="button" onClick={() => setDialog('withdraw')} disabled={client.depositBalance <= 0} className="py-3 rounded-xl bg-tg-secondary-bg text-tg-text font-semibold active:scale-[0.98] transition disabled:opacity-40">
            – {t('clients.withdraw')}
          </button>
        </div>
      )}
      ...
      {client.notes && (
        <div className="px-4 py-3 rounded-2xl bg-tg-secondary-bg mb-4">
          <p className="text-[11px] text-tg-hint uppercase tracking-wide mb-1">{t('clients.fields.notes')}</p>
          <p className="text-sm text-tg-text">{client.notes}</p>
        </div>
      )}

      <h2 className="text-xs text-tg-hint uppercase tracking-wider font-medium mb-2 px-1">{t('clients.history')}</h2>
      {transactions.length === 0 ? (
        <p className="text-sm text-tg-hint px-1">{t('clients.noTransactions')}</p>
      ) : (
      ...
      {canManage && (
        <div className="mt-8 pt-5 border-t border-tg-secondary-bg">
          {!confirmArchive ? (
            <button type="button" onClick={() => setConfirmArchive(true)} className="w-full py-3 rounded-xl text-tg-destructive text-sm font-medium active:bg-tg-secondary-bg transition">
              {t('clients.archive')}
            </button>
          ) : (
            <div className="flex flex-col gap-2">
              <p className="text-xs text-tg-hint text-center">{t('clients.archiveHint')}</p>
              <button type="button" onClick={handleArchive} disabled={archiving} className="w-full py-3 rounded-xl bg-tg-destructive/10 text-tg-destructive font-semibold active:scale-[0.98] transition disabled:opacity-50">
                {archiving ? t('common.loading') : t('clients.archiveConfirm')}
              </button>
              <button type="button" onClick={() => setConfirmArchive(false)} className="w-full py-2.5 rounded-xl bg-tg-secondary-bg text-tg-hint text-sm">
                {t('clients.cancel')}
              </button>
            </div>
          )}
          ...
          {(['Cash', 'Card', 'BankTransfer'] as const).map(m => (
            <button
              ...
              className={[
                ...
                method === m ? 'bg-tg-button text-tg-button-text' : 'bg-tg-secondary-bg text-tg-hint',
              ].join(' ')}
            >
